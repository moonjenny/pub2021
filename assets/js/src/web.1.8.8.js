...
var _MAKERS_ = {
    DEVICE: {
        ...
        isPc: __common_var_plugin__.device[0],
        isKakaoCommonApp: __common_var_plugin__.device[3],
        isKakaoMakersApp: __common_var_plugin__.device[4],
        isMakersApp: __common_var_plugin__.device[7],
        isMakersAppAnd:__common_var_plugin__.device[8],
        isMakersAppIos: __common_var_plugin__.device[9]
    },
    STATE: {
    ...
    CONFIG: {
        OVERRIDE_FOOTER_EVENT: function(bOverride){
            ...
            if(override){
                if(!__footer_improvement__.included){
                    console.warn('Override footer event needs footer_improvement vm');
                }
            }
...
_MAKERS_.NATIVE = (function(){
    ...
    var _ANDROID_INVOKER_ = {
        refreshOff: function(){
            makers.refreshOff();
        },
        showItemImage: function(iIndex, aImageUrls){
            var index = iIndex || 0;
            var imageUrls = aImageUrls || [];
            makers.showItemImage(index, imageUrls);
        },
        setTitle: function(sTitle){
            var title = sTitle || '';
            makers.setTitle(title);
        },
        hideTitle: function(){
            makers.hideTitle();
        },
    ...
    var _IOS_INVOKER_ = {
        refreshOff: function(){
            webkit.messageHandlers.bridge.postMessage({ name: 'refreshOff' });
        },
        showItemImage: function(iIndex, aImageUrls){
            var index = iIndex || 0;
            var imageUrls = aImageUrls || [];
            webkit.messageHandlers.bridge.postMessage({ name: 'showItemImage', index: index, imageUrls: imageUrls });
        },
        setTitle: function(sTitle){
            var title = sTitle || '';
            webkit.messageHandlers.bridge.postMessage({ name: 'setTitle', title:  title });
        },
        hideTitle: function(){
            webkit.messageHandlers.bridge.postMessage({ name: 'hideTitle' });
        },
        openUrl: function(sUrl){
            var url = sUrl || '';
            webkit.messageHandlers.bridge.postMessage({ name: 'openUrl', url: url });
        },
        outlink: function(sUrl){
            var url = sUrl || '';
            webkit.messageHandlers.bridge.postMessage({ name: 'outlink', url: url });
        },
        close: function(){
            webkit.messageHandlers.bridge.postMessage({ name: 'close' });
        },
        back: function(){
            webkit.messageHandlers.bridge.postMessage({ name: 'back' });
        }
    };
    ...
    try {
        ...
        if (tiaraPath != null) {
            if ($.isNumeric(tiaraPath)) {
                ...
                eventArr.push(tiaraPath);
                if (tiaraValue != null)
                    eventArr.push(tiaraValue);
                else if ($.isNumeric(tiaraPath))
                    eventArr.push(tiaraPath);
            }
        } else {
            if ($.isNumeric(tiaraPath)) {
...
function goPlusFriendScheme(pf_type) {
    closeConfirmLayer();
    closeFriendLayer();
    $.ajax({
        url : "/my/plus_friend_scheme.json",
        data : {
            'pf_type' : pf_type
        },
        type : "POST",
        ...
        success : function(json) {
            if (json.status == 0) {
                location.href = json.scheme;
            } else {
                openAlertLayer("플러스친구 호출이 실패하였습니다.<br>잠시후 다시 시도 부탁드립니다.");
            }
        },
        error : function(e1, e2) {
            openAlertLayer('오류가 발생하였습니다.\n\n창을 닫고 다시 시도해주세요.');
...
function goMassOrderPlusFriend() {

    function moveToGoPlusFriendScheme(){
        return goPlusFriendScheme("mass_order");
    }

    function openMassOrderPcTalk(){
        window.open('https://pf.kakao.com/_zExbvd');
    }

    if(_MAKERS_.DEVICE.isPc === true){
        return openMassOrderPcTalk();
    }

    return moveToGoPlusFriendScheme();
}
...
$('a[data-makers-href]').on('click', function(e){
    ...
    var isMakersAppScheme = rawMakersHref.indexOf('makers://app') > -1 ? true : false;
